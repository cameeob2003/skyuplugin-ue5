...
TArray<double> ASkyManager::getMoon() {
	...
	moonAge = normalizeRadians(longitude * DEG_TO_RAD - sun->eclipticLongitude) * Psin / PI_TWO;
...
FEphemeris* ASkyManager::doCalc(TArray<double> pos, bool geocentric) {
	...
	double celestialHoursToEarthTime = 1.0 / (siderealDayLength * PI_TWO);

	// Make calculations for the meridian
	double transit_time1 = celestialHoursToEarthTime * normalizeRadians(ra - lst);
	double transit_time2 = celestialHoursToEarthTime * (normalizeRadians(ra - lst) - PI_TWO);
	...
	if (FMath::Abs(tmp) <= 1.0) {
		...
		double rise_time2 = celestialHoursToEarthTime * (normalizeRadians(ra - ang_hor - lst) - PI_TWO);
		double set_time2 = celestialHoursToEarthTime * (normalizeRadians(ra + ang_hor - lst) - PI_TWO);
...
{
	if (r < 0 && r >= -PI_TWO) return r + PI_TWO;
	if (r >= PI_TWO && r < 2 * PI_TWO) return r - PI_TWO;
	if (r >= 0 && r < PI_TWO) return r;

	r -= PI_TWO * FMath::FloorToFloat(r / PI_TWO);
	if (r < 0.) r += PI_TWO;
